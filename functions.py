...
def plot_confusion_matrix(matrix, classes, img_name):
    plt.figure(figsize=(16, 10))
    sns.heatmap(matrix, annot=True, fmt=".2f", cmap="Blues")
    plt.xticks(range(len(classes)), classes.values(), rotation=45)
    plt.yticks(range(len(classes)), classes.values())
    plt.title("Confusion Matrix")
    plt.savefig(f"imgs/{img_name}.png")
    plt.close()


def compare_facies_plot(logs, compadre, facies_colors, img_name):
    # make sure logs are sorted by depth
    logs = logs.sort_values(by="Depth")
    cmap_facies = colors.ListedColormap(
        facies_colors[0 : len(facies_colors)], "indexed"
